...
while read -r LINE; do
	...
	LINE=$(echo "${LINE}" | sed 's/^"//; s/"$//; s/\\n/ /g;s/+//g') # Remove quotes, newlines, and + from output
	FROM=$(echo "${LINE}" | awk -F "::" '{print $1}')
	STIME=$(echo "${LINE}" | awk -F "::" '{print $2}')
	MSGID=$(echo "${LINE}" | awk -F "::" '{print $3}')
	MSG=$(echo "${LINE}" | sed 's/\\"/"/g' | awk -F "::" '{print $4}' | chartotty)
	...
	if [[ -n "${ISPRINTED}" ]]; then
		...
		HTIME=$(date +"%Y-%m-%d %H:%M:%S %Z" --date 'TZ="GMT" '"${STIME}")
